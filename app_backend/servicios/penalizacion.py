...
def leer_paginado(conexion, pagina=1, limite=LIMITE_POR_DEFECTO, offset=None, filtros=None):
    ...
    if filtros is not None and filtros:
        ...
        if q is not None:
            ...
            condiciones += f"""(
                ...
                {OBJETO}.id_reserva = %(q)s OR
                {OBJETO}.motivo LIKE %(q_amplio)s
            )"""
            filtros["q"] = q
    ...
    {OBJETO}.activa,
    {OBJETO}.severidad,
    DATE_FORMAT({OBJETO}.fecha_inicio, '{FORMATO_FECHA}') as fecha_inicio,
    DATE_FORMAT({OBJETO}.fecha_fin, '{FORMATO_FECHA}') as fecha_fin,
    DATE_FORMAT({OBJETO}.fecha_inicio, '{FORMATO_HORA}') as hora_inicio,
    DATE_FORMAT({OBJETO}.fecha_fin, '{FORMATO_HORA}') as hora_fin
    FROM {OBJETO}
    {clausula_where}
    LIMIT %(limite)s
    OFFSET %(offset)s
