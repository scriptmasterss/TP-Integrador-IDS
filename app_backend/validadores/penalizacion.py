"""validadores para penalizacion."""

import validadores.general

LLAVES_OBLIGATORIAS = ["id_usuario", "id_reserva", "motivo", "fecha_inicio", "fecha_fin", "activa", "severidad"]

MAPA_VALIDADORES = {
    "id_usuario": validadores.general.numero_positivo,
    "id_reserva": validadores.general.numero_positivo,
    "motivo": validadores.general.texto,
    "fecha_inicio": validadores.general.fecha,
    "fecha_fin": validadores.general.fecha,
...
NO_ES_SEVERIDAD_VALIDA = "valor no es baja, alta, o media"
...
def validar_existente(valor):
    ...
    if llave in valor.keys():
        ...
        if valor_llave not in ENUM_SEVERIDAD:
            return None, NO_ES_SEVERIDAD_VALIDA
        valor[llave] = valor_llave
