...
def get():
    ...
    with obtener_conexion() as conexion:
        ...
        filtros = validadores.qr.validar_filtro(filtros)

        resultados, total, error = servicios.qr.leer_paginado(conexion, pagina, limite, offset, filtros)
...
def get_uno(id):
    """Responde al GET /id."""
    with obtener_conexion() as conexion:
        resultado, error = servicios.qr.leer_uno(conexion, id)
        if resultado is None:
            if error == servicios.qr.SIN_ERROR:
...
def post():
    """Responde al POST."""
    with obtener_conexion() as conexion:
        nuevo_objeto, error = validadores.qr.validar_nuevo(request.get_json())
        if nuevo_objeto is None:
            return respuestas.peticion_incorrecta(error)

        resultado, error = servicios.qr.crear(conexion, nuevo_objeto)
        if resultado is None:
...
def put(id):
    ...
    with obtener_conexion() as conexion:
        nuevo_objeto, error = validadores.qr.validar_existente(request.get_json())
        if nuevo_objeto is None:
            return respuestas.peticion_incorrecta(error)

        resultado, error = servicios.qr.actualizar_uno(conexion, id, nuevo_objeto)
        if resultado is None:
...
def delete(id):
    """Responde al DELETE."""
    with obtener_conexion() as conexion:
        resultado, error = servicios.qr.eliminar_uno(conexion, id)
        if resultado is None:
...
def leer_uno(id):
    ...
    id_usuario = request.usuario.get("id_usuario")
    with obtener_conexion() as conexion:
        objeto, error = servicios.qr.leer_uno(conexion, id)
        if objeto is None:
            if error == servicios.qr.SIN_ERROR:
                return respuestas.no_encontrado(f"No se encontro {OBJETO} con ese id")
            else:
        ...
        if objeto.get("id_usuario") != id_usuario:
            return respuestas.desautorizado(f"Este {OBJETO} no es tuya")
