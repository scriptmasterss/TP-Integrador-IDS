"""Implementa rutas para estado_devuelto."""
...
def get():
    ...
    with obtener_conexion() as conexion:
        ...
        resultados, total, error = servicios.estado_devuelto.leer_paginado(conexion, pagina, limite, offset, filtros)
...
def get_uno(id):
    """Responde al GET /id."""
    with obtener_conexion() as conexion:
        resultado, error = servicios.estado_devuelto.leer_uno(conexion, id)
        if resultado is None:
            if error == servicios.estado_devuelto.SIN_ERROR:
...
def post():
    """Responde al POST."""
    with obtener_conexion() as conexion:
        nuevo_objeto, error = validadores.estado_devuelto.validar_nuevo(request.get_json())
        if nuevo_objeto is None:
            return respuestas.peticion_incorrecta(error)

        resultado, error = servicios.estado_devuelto.crear(conexion, nuevo_objeto)
        if resultado is None:
...
def put(id):
    ...
    with obtener_conexion() as conexion:
        nuevo_objeto, error = validadores.estado_devuelto.validar_existente(request.get_json())
        if nuevo_objeto is None:
            return respuestas.peticion_incorrecta(error)

        resultado, error = servicios.estado_devuelto.actualizar_uno(conexion, id, nuevo_objeto)
        if resultado is None:
...
def delete(id):
    """Responde al DELETE."""
    with obtener_conexion() as conexion:
        resultado, error = servicios.estado_devuelto.eliminar_uno(conexion, id)
        if resultado is None:
