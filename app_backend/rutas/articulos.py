"""Implementa rutas para articulos."""
...
def get():
    ...
    with obtener_conexion() as conexion:
        ...
        resultados, total, error = servicios.articulos.leer_paginado(conexion, pagina, limite, offset, filtros)
...
def get_uno(id):
    """Responde al GET /id."""
    with obtener_conexion() as conexion:
        resultado, error = servicios.articulos.leer_uno(conexion, id)
        if resultado is None:
            if error == servicios.articulos.SIN_ERROR:
...
def post():
    """Responde al POST."""
    with obtener_conexion() as conexion:
        nuevo_objeto, error = validadores.articulos.validar_nuevo(request.get_json())
        if nuevo_objeto is None:
            return respuestas.peticion_incorrecta(error)

        resultado, error = servicios.articulos.crear(conexion, nuevo_objeto)
        if resultado is None:
...
def put(id):
    ...
    with obtener_conexion() as conexion:
        nuevo_objeto, error = validadores.articulos.validar_existente(request.get_json())
        if nuevo_objeto is None:
            return respuestas.peticion_incorrecta(error)

        resultado, error = servicios.articulos.actualizar_uno(conexion, id, nuevo_objeto)
        if resultado is None:
...
def delete(id):
    """Responde al DELETE."""
    with obtener_conexion() as conexion:
        resultado, error = servicios.articulos.eliminar_uno(conexion, id)
        if resultado is None:
